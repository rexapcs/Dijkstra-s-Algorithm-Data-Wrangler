...
//
// Title:           A testing class for CS400Graph
...
public class DWCS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    public DWCS400Graph() { vertices = new Hashtable<>(); }
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            double extendedDistance = extendBy.weight.doubleValue();
            this.distance = copyPath.distance + extendedDistance;
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        Vertex startingVertex = this.vertices.get(start);

        if(!containsVertex(start) || !containsVertex(end)) {
            throw new NoSuchElementException("elements are not in the graph");
        }
        Hashtable<NodeType, Vertex> visited = new Hashtable<>();

        Path path1 = new Path(startingVertex);
        PriorityQueue<Path> queue = new PriorityQueue<>();
        queue.add(path1);
        while(!queue.isEmpty()){
            Path shortest = queue.poll();
            NodeType start2 = shortest.start.data;
            NodeType end2 = shortest.end.data;
            if(end2.equals(end) && start2.equals(start)) {
                return shortest;
            }
            if(!visited.contains(shortest.end)){
                startingVertex = shortest.end;
                for(int i = 0; i < startingVertex.edgesLeaving.size(); i++) {
                    Path newPath = new Path(shortest, startingVertex.edgesLeaving.get(i));
                    Vertex end1 = newPath.end;
                    if(!visited.contains(end1)) {
                        queue.add(newPath);
                    }
                }
            }
            visited.put(shortest.end.data, shortest.end);
        }

        throw new NoSuchElementException("Path is not present in list");
