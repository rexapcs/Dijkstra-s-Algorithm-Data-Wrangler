...
public class CS400Graph<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
  ...
  protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    ...
    PriorityQueue<Path> targetPath = new PriorityQueue<>();
    ...
    while (!queue.isEmpty()) {
      ...
      Vertex v = currPath.end;

      if (currPath.end.data == end) {
        return currPath;
      }

      for (int i = 0; i < v.edgesLeaving.size(); i++) {
        // for (Edge e : v.edgesLeaving) {
        Edge e = v.edgesLeaving.get(i);

        if (!visited.contains(v)) {
          Path next = new Path(currPath, e);
          queue.add(next);
          if (next.start.data == start && next.end.data == end) {
            targetPath.add(next);
          }
        }
      }
      visited.put(v.data, v);
    }
